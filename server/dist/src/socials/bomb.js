...
exports.shareBombResult = shareBombResult;
...
async function getGameReplayHash(gameId) {
    try {
...
async function getGamePlayerInfo(gameId, round, playerId) {
    try {
        const [client, scoreRecord] = await Promise.all([
            prisma_1.prisma.gameClient.findFirst({
                where: { gameId, playerId },
                select: { name: true }
            }),
            prisma_1.prisma.gameScore.findUnique({
                where: {
...
async function shareBombResult(gameId, round, playerId, img) {
